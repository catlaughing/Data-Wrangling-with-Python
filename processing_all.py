...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in tr:
            td = row.find_all('td')
            if td[1].text != 'TOTAL':
                temp['courier'] = info['courier']
                temp['airport'] = info['airport']
                temp["year"] = int(td[0].text)
                temp['month'] = int(td[1].text)
                temp['flights'] = {"domestic" : int(td[2].text.replace(',','')), "international" : int(td[3].text.replace(',',''))}
                data.append(temp)
